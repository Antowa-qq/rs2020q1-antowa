...
export default class Game {
    ...
    isCorrectWord(word) {
        let desiredСard = '';
        for (let nameSection in this.storageCards) {
            this.storageCards[nameSection].forEach(item => {
                if (item.word === this.currentWord && item.translation === this.cardList[this.currentWordIndex].translation) {
                    desiredСard = item;
                }
            });
        }
        ...
        desiredСard.incorrect++;
        localStorage.setItem('statistics', JSON.stringify(this.storageCards));

        this.unCorrectCounter++;
        this.playErrorAudio();
